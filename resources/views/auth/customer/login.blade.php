                            ...
                            <div class="card border-grey border-lighten-3 m-0">
                                <div class="card-header border-0">
                                    <div class="card-title text-center">
                                        <div class="p-1">
                                            <img src="{{asset('app-assets/images/logo/logo.png')}}" alt="" style="width: 20%; margin-bottom: 10px;">
                                    ...
                                    <div class="card-body">
                                        ...
                                        @if(count( $errors ) > 0)
                                            @foreach ($errors->all() as $error)
                                               <h6 class="danger text-center pb-1">{{ $error }}</h6>
                                            @endforeach
                                        @endif
                                          <form class="form-horizontal form-simple" method="POST" action="{{ route('customer.auth') }}" aria-label="{{ __('Login') }}" novalidate>
                                            @csrf
                                            <fieldset class="form-group position-relative has-icon-left mb-0">
                                                <input class="form-control" id="code" placeholder="Client Code" type="text" name="code" required autofocus>
                                            ...
                                            <fieldset class="form-group position-relative has-icon-left">
                                                <input id="password" class="form-control" placeholder="Client Password" type="password" name="password" required autocomplete="current-password" >
