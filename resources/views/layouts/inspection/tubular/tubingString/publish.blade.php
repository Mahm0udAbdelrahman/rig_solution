...
@section('content')
    <section id="validation">
        <div class="row">
            <div class="col-12">
                {!! Form::model($model, ['route' => ['tubingString.publishSubmit', $model->id], 'method' => 'PATCH', 'class'=> 'steps-validation wizard-circle']) !!}
                {{ csrf_field() }}
                @include('layouts.inspection.tubular.tubingString.form');
                {!! Form::close() !!}
            </div>
        </div>
    </section>
...
@push('bottom-child-scripts')
    ...
    <script>
      function initTubingStringRepeaters() {
        $('.equipment-repeater, .inspection-data-repeater').each(function () {
          var $repeater = $(this);
          ...
          $repeater.repeater({
            show: function () {
              $(this).slideDown();
              $(this).find('.equipment-used').trigger('change');
            },
