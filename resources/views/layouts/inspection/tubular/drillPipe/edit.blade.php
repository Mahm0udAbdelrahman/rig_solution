...
@section('content')
    <section id="validation">
        <div class="row">
            <div class="col-12">
                {!! Form::model($model, ['route' => ['drillPipe.update', $model->id], 'method' => 'PATCH', 'class'=> 'steps-validation wizard-circle']) !!}
                {{ csrf_field() }}
                @include('layouts.inspection.tubular.drillPipe.form');
                {!! Form::close() !!}
            </div>
        </div>
    </section>
...
@push('bottom-child-scripts')
    ...
    <script>
      ...
      $('.steps-validation').on('ifUnchecked', '.inspection_method', function (event) {
        var id =  $(this).data('id');
        if (id === 'other') {
          $('#other_inspection_method').val(null);
