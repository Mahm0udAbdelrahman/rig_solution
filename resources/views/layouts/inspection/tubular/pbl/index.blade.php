...
@section('content')
    <section class="users-list-wrapper">
        <div class="users-list">
            @if ($reports == 0)
                @include('layouts.repeated.nodata', ['route' => 'pbl'])
            @else
{{--                @can('create', 'App\Models\Inspection\Tubular\Pbl')--}}
                    <a href="{{route('pbl.create')}}" class="btn btn-primary clear"><i class="la la-plus"></i> Create
...
@endsection
@if ($reports != 0)
    @include('layouts.scripts.datatables', ['route' => 'pbl', 'columns' => ['code', 'client', 'client_department','deploc', 'identification_no', 'action'],
