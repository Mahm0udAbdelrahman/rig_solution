...
@section('content')
    <section id="validation">
        <div class="row">
            <div class="col-12">
                {!! Form::model($model, ['route' => ['pbl.publishSubmit', $model->id], 'method' => 'PATCH', 'class'=> 'steps-validation wizard-circle', 'enctype'=>'multipart/form-data']) !!}
                {{ csrf_field() }}
                @include('layouts.inspection.tubular.pbl.form')
                {!! Form::close() !!}
            </div>
        </div>
    </section>
...
// Needed in some cases if the user went back (clean up)
      ...
      if (currentIndex === 2 && newIndex === 3) {
        var $wizard = $(this);
        var $inspectionDescription = $wizard.find('input[name="inspection_description"]').first();
        var $description = $wizard.find('input[name="description"]').first();
        var $identificationNo = $wizard.find('input[name="identification_no"]').first();

        var inspectionDescriptionValue = $.trim(($inspectionDescription.val() || '').toString());
        var descriptionValue = $.trim(($description.val() || '').toString());
        var identificationNoValue = $.trim(($identificationNo.val() || '').toString());

        if (inspectionDescriptionValue !== '' && descriptionValue !== '' && identificationNoValue !== '') {
          return true;
        }

        var $firstMissing = inspectionDescriptionValue === '' ? $inspectionDescription
          : (descriptionValue === '' ? $description : $identificationNo);
      ...
      if (currentIndex === 3 && newIndex === 4) {
        var $wizard = $(this);
        var $pipeType = $wizard.find('select[name="standards[type]"]').first();
        var pipeTypeValue = $.trim(($pipeType.val() || '').toString());
        if (pipeTypeValue !== '') {
          return true;
        }
        if (window.showWizardValidationMessage) {
          window.showWizardValidationMessage($wizard, $pipeType);
        }
        return false;
      }
