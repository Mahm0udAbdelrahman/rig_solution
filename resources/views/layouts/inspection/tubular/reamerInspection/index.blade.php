...
@section('content')
    <section class="users-list-wrapper">
        <div class="users-list">
            @if ($reports == 0)
                @include('layouts.repeated.nodata', ['route' => 'reamerInspection'])
            @else
                @can('create', 'App\Models\Inspection\Tubular\ReamerInspection')
                    <a href="{{route('reamerInspection.create')}}" class="btn btn-primary clear"><i class="la la-plus"></i> Create
...
@endsection
@if ($reports != 0)
    @include('layouts.scripts.datatables', ['route' => 'reamerInspection', 'columns' => ['code', 'client', 'client_department','deploc','material_no','material_description', 'action'],
