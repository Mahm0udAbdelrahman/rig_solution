...
@section('content')
    <section class="users-list-wrapper">
        <div class="users-list">
            @if ($reports == 0)
                @include('layouts.repeated.nodata', ['route' => 'stabilizerInspection'])
            @else
                @can('create', 'App\Models\Inspection\Tubular\StabilizerInspection')
                    <a href="{{route('stabilizerInspection.create')}}" class="btn btn-primary clear"><i class="la la-plus"></i> Create
...
@endsection
@if ($reports != 0)
    @include('layouts.scripts.datatables', ['route' => 'stabilizerInspection', 'columns' => ['code', 'client', 'client_department','deploc','tool_number','material_description', 'action'],
