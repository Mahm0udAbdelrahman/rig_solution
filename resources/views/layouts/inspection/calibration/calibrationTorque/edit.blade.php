...
@section('content')
    <section id="validation">
        <div class="row">
            <div class="col-12">
                {!! Form::model($model, ['route' => ['calibrationTorque.update', $model->id], 'method' => 'PATCH', 'class'=> 'steps-validation wizard-circle', 'enctype'=>'multipart/form-data']) !!}
                {{ csrf_field() }}
                @include('layouts.inspection.calibration.calibrationTorque.form');
                {!! Form::close() !!}
            </div>
        </div>
    </section>
