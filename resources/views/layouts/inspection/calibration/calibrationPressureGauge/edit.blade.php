...
@section('content')
    <section id="validation">
        <div class="row">
            <div class="col-12">
                {!! Form::model($model, ['route' => ['calibrationPressureGauge.update', $model->id], 'method' => 'PATCH', 'class'=> 'steps-validation wizard-circle', 'enctype'=>'multipart/form-data']) !!}
                {{ csrf_field() }}
                @include('layouts.inspection.calibration.calibrationPressureGauge.form');
                {!! Form::close() !!}
            </div>
        </div>
    </section>
