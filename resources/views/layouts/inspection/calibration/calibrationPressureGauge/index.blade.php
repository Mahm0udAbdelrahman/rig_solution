...
@section('content')
    <section class="users-list-wrapper">
        <div class="users-list">
            @if ($reports == 0)
                @include('layouts.repeated.nodata', ['route' => 'calibrationPressureGauge'])
            @else
{{--                @can('create', 'App\Models\Inspection\Calibration\CalibrationPressureGauge')--}}
                    <a href="{{route('calibrationPressureGauge.create')}}" class="btn btn-primary clear"><i class="la la-plus"></i> Create
...
@endsection
@if ($reports != 0)
    @include('layouts.scripts.datatables', ['route' => 'calibrationPressureGauge', 'columns' => ['code', 'client', 'client_department','deploc', 'action'],
