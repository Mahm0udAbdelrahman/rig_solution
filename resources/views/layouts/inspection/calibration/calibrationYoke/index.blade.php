...
@section('content')
    <section class="users-list-wrapper">
        <div class="users-list">
            @if ($reports == 0)
                @include('layouts.repeated.nodata', ['route' => 'calibrationYoke'])
            @else
{{--                @can('create', 'App\Models\Inspection\Calibration\CalibrationYoke')--}}
                    <a href="{{route('calibrationYoke.create')}}" class="btn btn-primary clear"><i class="la la-plus"></i> Create
...
@endsection
@if ($reports != 0)
    @include('layouts.scripts.datatables', ['route' => 'calibrationYoke', 'columns' => ['code', 'client', 'client_department','deploc', 'action'],
