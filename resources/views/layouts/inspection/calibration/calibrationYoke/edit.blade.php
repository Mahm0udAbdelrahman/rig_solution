...
@section('content')
    <section id="validation">
        <div class="row">
            <div class="col-12">
                {!! Form::model($model, ['route' => ['calibrationYoke.update', $model->id], 'method' => 'PATCH', 'class'=> 'steps-validation wizard-circle', 'enctype'=>'multipart/form-data']) !!}
                {{ csrf_field() }}
                @include('layouts.inspection.calibration.calibrationYoke.form');
                {!! Form::close() !!}
            </div>
        </div>
    </section>
