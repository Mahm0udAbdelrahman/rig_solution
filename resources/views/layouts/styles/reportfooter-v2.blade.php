@php
    ...
    if ($user_id_approved != Null) {
        $approvedUser = App\Models\User::with('employee')->find($user_id_approved);
        $approvedEmployee = optional($approvedUser)->employee;
    }
@endphp

@if(!str_contains(get_class($model), 'Calibration'))
...
<div class="row row-flex jjccf" >
    <div class="col-3 p-0 border-dark black" style="padding-top: 0 !important;">
        ...
        <p class="pl-1" style="padding-top: 5px; padding-bottom: 5px;">Signature:
            <img class="media-object" style="width: 130px; height: 45px; margin: auto;" src="{{Storage::url('employees/')}}{{$esign}}" alt="" />
        </p>
    </div>
    <div class="col-3 p-0 border-dark" style="padding-top: 0 !important;">
        <h6 class="bg-dark white text-center text-bold-600 mb-0 border-dark"> Stamp</h6>
        @if($user_id_approved!=Null)
            <div class="text-center" style="display: flex; justify-content: center; padding: 1em 0;">
                <img src="{{asset('app-assets/images/logo/stamp.jpeg')}}" style="width:13em; height: 10em; position: relative;"/>
            </div>
        @endif
    </div>
    ...
    <div class="col-4 p-0 border-dark" style="padding-top: 0 !important;">
        ...
        <div class="text-center black">
	          @if($user_id_approved==Null)
                    ...
                    @endif
	          @else
                    <p class="pl-1 mb-0 text-16 black mt-0" style="display: flex; font-weight: 600; flex-direction: row;">
                        <span style="padding-top: 3em;">{{ optional($approvedEmployee)->name }} </span>
                        <img class="media-object" style="width: 130px; height: 65px;  margin: auto;" src="{{Storage::url('employees/')}}{{ optional($approvedEmployee)->esign }}" alt="" />
                    </p>
                <p class="pl-1 mb-0 text-16 black mt-0" style="display: flex; padding-top: 30px; font-weight: 600; flex-direction: row;">
                        <span style="width: 50%;">Name</span>
                        <span style="width: 50%;">Signature</span>
                    </p>
	          @endif
        </div>
    </div>
