...
<section class="users-list-wrapper">
    <div class="card">
        <div class="card-content">
            <div class="card-body">
                <form method="POST" action="{{ route('accountant.store') }}" id="accountant-entry-form">
                    @csrf
                    @if($errors->any())
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="job_request_id">
                                    ...
                                    @foreach(($job_request_options ?? []) as $jobRequestId => $jobRequestLabel)
                                        <option value="{{ $jobRequestId }}" {{ (string)old('job_request_id', $prefill_job_request_id ?? optional($job_request)->id) === (string)$jobRequestId ? 'selected' : '' }}>
                                            {{ $jobRequestLabel }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="invoice_id">
                                    ...
                                    @foreach(($invoice_options ?? []) as $invoiceId => $invoiceLabel)
                                        <option value="{{ $invoiceId }}" {{ (string)old('invoice_id', $prefill_invoice_id ?? null) === (string)$invoiceId ? 'selected' : '' }}>
                                            {{ $invoiceLabel }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="payment_id">
                                    ...
                                    @foreach(($payment_options ?? []) as $paymentId => $paymentLabel)
                                        <option value="{{ $paymentId }}" {{ (string)old('payment_id', $prefill_payment_id ?? null) === (string)$paymentId ? 'selected' : '' }}>
                                            {{ $paymentLabel }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 col-12">
                            <div class="form-group">
                                ...
                                <input type="text" name="reference_no" class="form-control" value="{{ old('reference_no', $prefill_reference_no ?? '') }}" placeholder="Optional reference">
                            </div>
                        </div>
                        <div class="col-md-3 col-12">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="entry_type" required>
                                    ...
                                    <option value="accrual" {{ old('entry_type', $prefill_entry_type ?? null) === 'accrual' ? 'selected' : '' }}>Accrual</option>
                                    <option value="cash" {{ old('entry_type', $prefill_entry_type ?? null) === 'cash' ? 'selected' : '' }}>Cash</option>
                                    <option value="adjustment" {{ old('entry_type', $prefill_entry_type ?? null) === 'adjustment' ? 'selected' : '' }}>Adjustment</option>
                                    <option value="invoice" {{ old('entry_type', $prefill_entry_type ?? null) === 'invoice' ? 'selected' : '' }}>Invoice (Legacy)</option>
                                    <option value="payment" {{ old('entry_type', $prefill_entry_type ?? null) === 'payment' ? 'selected' : '' }}>Payment (Legacy)</option>
                                    <option value="expense" {{ old('entry_type', $prefill_entry_type ?? null) === 'expense' ? 'selected' : '' }}>Expense</option>
                                    <option value="bank" {{ old('entry_type', $prefill_entry_type ?? null) === 'bank' ? 'selected' : '' }}>Bank</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-3 col-12">
                            <div class="form-group">
                                ...
                                <input type="text" name="currency" class="form-control" value="{{ old('currency', $prefill_currency ?? 'USD') }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-3 col-12">
                            <div class="form-group">
                                ...
                                <input type="text" id="entry-total-debit" class="form-control" value="{{ old('amount', $prefill_amount ?? '') }}" readonly>
                            </div>
                        </div>
                        <div class="col-md-3 col-12">
                            <div class="form-group">
                                <label>Total Credit</label>
                                <input type="text" id="entry-total-credit" class="form-control" value="{{ old('amount', $prefill_amount ?? '') }}" readonly>
                            </div>
                        </div>
                        <div class="col-md-3 col-12">
                            <div class="form-group">
                                <label>Legacy Amount (auto)</label>
                                <input type="text" name="amount" id="entry-legacy-amount" class="form-control" value="{{ old('amount', $prefill_amount ?? '') }}" readonly>
                            </div>
                        </div>
                        <div class="col-md-3 col-12">
                            <div class="form-group">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="form-group">
                                <label>Note</label>
                                <textarea class="form-control" name="note" rows="4">{{ old('note', $prefill_note ?? '') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex mt-1">
                        <button type="submit" class="btn btn-primary mr-1">Save Entry</button>
                        <a href="{{ route('accountant.index') }}" class="btn btn-light border">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
