															              ...
															              <div class="form-group">
																				...
																				<div class="col-md-4">
																						<div class="form-group">
																								<div class="controls">
																										<label>Date</label>
																										<input type="text" name="date" class="form-control dp-date-range-from" placeholder="Date" aria-invalid="false" required="" data-validation-required-message="This field is required" value="{{date('d-m-Y', strtotime($invoice->created_at->toDateString()))}}"  />
																								</div>
																						</div>
																				</div>
																		</div>
																				...
																				<div class="col-md-12">
																						<div class="form-group">
																								<div class="controls">
																										...
																										<input type="text" name="cpo" class="form-control" placeholder="Client P.O code" value="{{$invoice->cpo}}" />
																								</div>
																						</div>
																				</div>
																		</div>
																				...
																				<div class="table-responsive col-12">
																						<table class="table titems">
																								...
																								<tbody data-repeater-list="items">
																										@foreach(json_decode($invoice->items) as $qutationitem)
																				...
																				<div class="col-7 repeater">
																						<div class="repeater">
																								...
																								<div data-repeater-list="payments">
																										@foreach(json_decode($invoice->terms) as $method)
																						...
																						<div class="table-responsive">
																								<table class="table">
																										<tbody>
																												<tr>
																														<td>Sub Total</td>
																														<td class="text-right subtotal"> {{$invoice->sub_total}} <span class="months">{{$invoice->type}}</span></td>
																												</tr>
																												<tr class="warning">
																														<td class="text-bold-800">
																																...
																																<div style="display: flex; align-items: center; float: left; margin-left: 15px;">
																																		<select name="discount_type" class="form-control discount_type" style="width: 85px; padding: 2px 5px; height: 32px; margin-right: 5px;">
																																				<option @if(($invoice->discount_type ?? 'percentage') == 'percentage') selected @endif value="percentage">%</option>
																																				<option @if(($invoice->discount_type ?? 'percentage') == 'amount') selected @endif value="amount">Amount</option>
																																		</select>
																																		<input type="number" step="0.01" min="0" name="discount_value" class="form-control discount_value" style="width: 85px; height: 32px; padding: 2px 5px;" value="{{$invoice->discount_value ?? 0}}" placeholder="0">
																																</div>
																														</td>
																														<td class="text-bold-800 text-right discount_amount">- {{number_format($invoice->discount_amount ?? 0, 2, '.', '')}} <span class="months">{{$invoice->type}}</span></td>
																												</tr>
																												<tr class="success">
																														<td>
																																<div class="form-group mb-0">
																																		<div class="controls">
																																				<label style="margin-left: 0 !important;">TAX <span class="months1">(@if($invoice->tax == 0) 0% @else 14% @endif)</span></label>
																																				<div style="display: inline-block; margin-left: 10px;">
																																						<input type="checkbox" class="switchery mr-1" id="suporcli1" name="suporcli1" @if($invoice->tax != 0) checked @endif />
																																				</div>
																																		</div>
																																</div>
																														</td>
																														<td class="text-right tax"> {{$invoice->tax}} <span class="months">{{$invoice->type}}</span></td>
																												</tr>
																												<tr class="pink">
																														<td class="text-bold-800">
																																...
																																<select name="discountTax" class="form-control discountTax" style="width: 60px; padding-left: 0; float: left; margin-left: 15px;" required>
																																		<option @if($invoice->withholding == 0) selected @endif value="0">0%</option>
																																		<option @if($invoice->withholding == 1) selected @endif value="1">1%</option>
																																		<option @if($invoice->withholding == 3) selected @endif value="3">3%</option>
																																</select>
																														</td>
																														<td class="text-bold-800 text-right discountTaxAfter">- {{($invoice->sub_total*$invoice->withholding)/100}} <span class="months">{{$invoice->type}}</span></td>
																												</tr>
																												<tr>
																														<td class="text-bold-800">Total</td>
																														<td class="text-bold-800 text-right total"> {{$invoice->total}} <span class="months">{{$invoice->type}}</span></td>
																												</tr>
																										</tbody>
																								</table>
																						</div>
								...
								$.ajax({
										...
										success: function (data){
												toastr.info('Good Job !', data.success, { positionClass: 'toast-bottom-left', "showMethod": "slideDown", "hideMethod": "slideUp", "progressBar": true, timeOut: 1000, fadeOut: 1000, onHidden: function () {
													window.location.replace("{{route('invoice.index', ['type'=> $invoice->invoice_company_type])}}");
												}  });
										},
								});
						}
				...
				function toggleAlreadyPaidSection(forceAmountSync) {
						...
						if (enabled) {
								...
								if (paymentDateInput.length && $.trim(paymentDateInput.val()) === '') {
										paymentDateInput.val('{{ old('payment_date', $existing_payment && $existing_payment->payment_date ? \Carbon\Carbon::parse($existing_payment->payment_date)->format('Y-m-d') : now()->format('Y-m-d')) }}');
								}
