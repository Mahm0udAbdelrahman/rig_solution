                ...
                <form method="POST" action="{{ route('expense.update', $expense->id) }}">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control {{ $errors->has('employee_id') ? 'is-invalid' : '' }}" name="employee_id" required>
                                    ...
                                    @foreach(($employee_options ?? []) as $employeeId => $employeeLabel)
                                        <option value="{{ $employeeId }}" {{ (string)old('employee_id', $expense->employee_id) === (string)$employeeId ? 'selected' : '' }}>
                                            {{ $employeeLabel }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="job_request_id">
                                    ...
                                    @foreach(($job_request_options ?? []) as $jobRequestId => $jobRequestLabel)
                                        <option value="{{ $jobRequestId }}" {{ (string)old('job_request_id', $expense->job_request_id) === (string)$jobRequestId ? 'selected' : '' }}>
                                            {{ $jobRequestLabel }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 col-12">
                            <div class="form-group">
                                <label>Amount</label>
                                <input type="text" name="amount" class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" value="{{ old('amount', number_format((float)$expense->amount, 2, '.', '')) }}" required>
                                @error('amount')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-3 col-12">
                            <div class="form-group">
                                ...
                                <input type="text" id="expense-currency" name="currency" class="form-control {{ $errors->has('currency') ? 'is-invalid' : '' }}" value="{{ old('currency', $expense->currency ?: 'USD') }}">
                        ...
                        <div class="col-md-3 col-12">
                            <div class="form-group">
                                <label>Status</label>
                                @php($statusValue = old('status', $expense->status ?: 'draft'))
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control {{ $errors->has('expense_account_id') ? 'is-invalid' : '' }}" name="expense_account_id">
                                    ...
                                    @foreach(($expense_account_options ?? []) as $accountId => $accountLabel)
                                        <option value="{{ $accountId }}" {{ (string)old('expense_account_id', $expense->expense_account_id) === (string)$accountId ? 'selected' : '' }}>
                                            {{ $accountLabel }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control {{ $errors->has('bank_account_id') ? 'is-invalid' : '' }}" id="expense-bank-account" name="bank_account_id">
                                    ...
                                    @foreach(($bank_options ?? []) as $bankId => $bankLabel)
                                        <option
                                            ...
                                            {{ (string)old('bank_account_id', $expense->bank_account_id) === (string)$bankId ? 'selected' : '' }}
                                        >
                                            {{ $bankLabel }}
                                        </option>
                                    @endforeach
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-8 col-12">
                            <div class="form-group">
                                <label>Note</label>
                                <textarea class="form-control" name="note" rows="3">{{ old('note', $expense->note) }}</textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-12">
                            <div class="form-group mb-0">
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input" id="is-posted" name="is_posted" value="1" {{ old('is_posted', (int)$expense->is_posted) ? 'checked' : '' }}>
                    ...
                    <div class="d-flex">
                        <button type="submit" class="btn btn-primary mr-1">Update Expense</button>
                        <a href="{{ route('expense.show', $expense->id) }}" class="btn btn-light border">Back</a>
                    </div>
                </form>
            </div>
        </div>
